...
模型是否支持多轮对话，并不由预训练阶段单独决定。更准确地说，预训练提供了语言能力与上下文建模基础，而多轮对话行为主要在 SFT/对齐阶段通过多轮样本进行塑形。如果要使模型支持多轮对话，我们需要在 SFT 时将训练数据构造成多轮对话格式，让模型能够利用之前的知识来生成回答。假设我们目前需要构造的多轮对话是：
