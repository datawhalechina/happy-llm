...
如果你是第一次系统学习大模型训练，建议先完整掌握本章的 Pretrain、SFT 与 PEFT 内容，再进入偏好对齐实践。在实践层面，推荐优先从 DPO 这类更轻量的偏好优化方法入手，再逐步理解奖励模型和 RLHF 的完整链路。这样可以在不显著增加工程复杂度的前提下，建立对后训练流程的整体认识。

### 6.4.5 偏好数据集构建

在构建奖励模型或进行 DPO 训练之前，我们首先需要准备高质量的人类反馈数据集。此数据集的核心目标是为每条给定的提示（prompt）提供多个候选回答（completion），并由人类标注者对这些回答进行细致的评定与排序。通过对回答的对比和筛选，我们得以为机器模型提供明确的参考标准，帮助其进一步学习在给定任务下如何生成更符合人类期望的输出。
...
奖励模型训练通常使用成对偏好数据。对于同一个 `prompt`，数据集中会包含一个更符合人类偏好的回答 `chosen`，以及一个相对较差的回答 `rejected`。奖励模型的目标不是直接生成文本，而是学习给 `chosen` 更高的分数、给 `rejected` 更低的分数。完成训练后，奖励模型可以接入 RLHF 的 PPO 阶段，为策略模型生成的回答提供奖励信号。

在 TRL 中，可以使用 `RewardTrainer` 训练奖励模型。下面的代码展示了一个最小示例，真实训练时需要将 `model_name`、数据路径、batch size 和训练轮数替换为自己的配置。
...
train_dataset = Dataset.from_list([
    {
        "chosen": "Python 中的列表是一种有序、可变的容器，可以存储多个元素。",
        "rejected": "Python 中的列表就是数据。",
    },
    {
        "chosen": "元组是有序但不可变的容器，创建后不能直接修改其中的元素。",
        "rejected": "元组也可以存东西。",
    },
])
...
需要注意的是，奖励模型本身只是 RLHF 流程中的一部分。完整 RLHF 往往还需要策略模型、参考模型、奖励模型和 PPO 优化过程，因此工程复杂度较高。如果学习目标是先跑通偏好对齐流程，可以优先从 DPO 入手。

### 6.4.7 DPO 直接偏好优化

DPO（Direct Preference Optimization）是一种更轻量的偏好对齐方法。它不显式训练奖励模型，也不需要像 PPO 那样在线采样并计算奖励，而是直接利用 `prompt`、`chosen`、`rejected` 三元组优化策略模型，使模型相对于参考模型更倾向于生成 `chosen`。

与 RLHF 相比，DPO 的工程流程更接近前文介绍的 SFT 和 LoRA 微调：仍然是加载模型、准备数据、设置训练参数、调用 Trainer。差异主要在于训练数据从单条监督答案变成了成对偏好答案，损失函数也从普通的语言建模损失变成了偏好优化损失。
...
train_dataset = Dataset.from_list([
    {
        "prompt": "请解释 Python 中的列表。",
        "chosen": "Python 中的列表是一种有序、可变的容器，可以存储多个元素，并支持索引访问。",
        "rejected": "列表就是用来存东西的。",
    },
    {
        "prompt": "请解释 Python 中的元组。",
        "chosen": "元组是一种有序、不可变的容器，适合保存不希望被修改的数据。",
        "rejected": "元组和列表差不多。",
    },
])
...
如果显存有限，可以结合第 6.3 节介绍的 LoRA/QLoRA，只训练少量适配器参数。实践时建议先用几十条样本检查数据字段、模板拼接和 loss 是否正常下降，再扩大到完整数据集。

### 6.4.8 偏好对齐实践检查清单

在真正开始偏好对齐训练前，可以按下面的顺序检查：

1. 先确认 SFT 模型已经具备基本指令跟随能力，否则偏好对齐很难直接弥补基础能力不足。
2. 检查偏好数据是否围绕同一个 `prompt` 构造成对回答，并确保 `chosen` 的质量确实稳定优于 `rejected`。
3. 先用小样本跑通 `RewardTrainer` 或 `DPOTrainer`，确认 tokenizer、padding、截断长度和显存占用没有问题。
4. 训练后不要只看 loss，还要固定一组验证问题，对比 SFT 模型和偏好对齐模型的回答质量。
5. 如果模型回答变得过短、过度拒答或模板化，需要回到数据分布、`beta` 参数、学习率和训练轮数上排查。
...
[5] TRL RewardTrainer 官方文档. https://huggingface.co/docs/trl/reward_trainer

[6] TRL DPOTrainer 官方文档. https://huggingface.co/docs/trl/dpo_trainer

[7] Rafael Rafailov, Archit Sharma, Eric Mitchell, Christopher D. Manning, Stefano Ermon, and Chelsea Finn. (2023). *Direct Preference Optimization: Your Language Model is Secretly a Reward Model.* arXiv preprint arXiv:2305.18290.

[8] Long Ouyang, Jeffrey Wu, Xu Jiang, Diogo Almeida, Carroll Wainwright, Pamela Mishkin, Chong Zhang, Sandhini Agarwal, Katarina Slama, Alex Ray, et al. (2022). *Training language models to follow instructions with human feedback.* arXiv preprint arXiv:2203.02155.
