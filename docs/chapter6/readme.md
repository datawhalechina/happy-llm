...
- [`第六章 大模型训练流程实践`](./第六章%20大模型训练流程实践.md)：第六章正文，介绍训练框架、数据处理、Trainer、DeepSpeed、LoRA/QLoRA 与偏好对齐等核心内容。
...
4. 完成上述内容后，再阅读正文 `6.4 偏好对齐`，建立从 SFT 到 Post-Training 的完整认识。
...
第六章正文已经覆盖了核心训练主线。若你希望进一步理解 RLHF、DPO、KTO 与奖励模型，可以在完成 Pretrain、SFT 与 PEFT 后继续阅读正文 `6.4 偏好对齐`，而不必在第一次阅读时一次性掌握所有后训练细节。
